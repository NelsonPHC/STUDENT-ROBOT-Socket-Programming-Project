...
int main(){
	...
	// Connect to server (i.e., student) (Now robot is the client).
...
//step7
char rcvBuff[1024];
recv( s2, rcvBuff, sizeof(rcvBuff), 0 );

printf("receive-buffer information from STUDENT: %s\n", rcvBuff);

int j;
for(j=0;j<=strlen(rcvBuff)-1;j++){
	if(rcvBuff[j]=='b')
		rcvBuff[j]=0;		//now rcvbuff information is decoded such that it only contains digits
}
...
char packet[n]={0};		//set packet size = receive buffer size

printf("Sending as much packets as possible in 30 secs...");			//afterwards: sending as many packets as possible in 30 seconds...

time_t startingTime, currentTime;
int i=0;
do{
	if(send(s2, packet, sizeof(packet), 0)){		//send packet of n bytes
...
//step8
char rcvBuffSet[1024];
recv( s2, rcvBuffSet, sizeof(rcvBuffSet), 0 );

printf("receive-buffer information from STUDENT: %s\n", rcvBuffSet);

int k;
for(k=0;k<=strlen(rcvBuffSet)-1;k++){
	if(rcvBuffSet[k]=='b')
		rcvBuffSet[k]=0;		//now rcvbuff information is decoded such that it only contains digits
}

int n1 = atoi(rcvBuffSet);

printf("%d\n", n1);

printf("Sending as much packets as possible in 30 secs after new receive buffer size...");

char packet1[n1]={0};		//set packet size = receive buffer size
time_t startingTime1, currentTime1;
int a=0;
do{
	if(send(s2, packet1, sizeof(packet1), 0)){		//send packet of n bytes
